...
public class braceletScript extends Script {
        ...
        mainScheduledFuture = scheduledExecutorService.scheduleWithFixedDelay(() -> {
            ...
            try {
                if (!Microbot.hasLevel(levelRequired, Skill.CRAFTING)) {
                    shutdown("Crafting level to low to craft " + config.braceletType().getName());
                    return;
                }
    ...
    public void collectFromBank() {
        ...
        if (!Rs2Inventory.isFull() && !Rs2Inventory.hasItemAmount(gem, 13)) {
            if (braceletType.equals(goldBracelet)) {
                if (!Rs2Bank.hasItem(bar)) {
                    shutdown("Out of gold bars");
                    return;
                }
            ...
            if(!Rs2Bank.hasItem(bar) || !Rs2Bank.hasItem(gem)) {
                shutdown("Out of materials");
                return;
            }
    ...
    public void smelt() {
        ...
        if (Rs2Widget.hasWidget("What would you like to make?")) {
            ...
            sleepUntil(() -> Microbot.isGainingExp, 2000);
